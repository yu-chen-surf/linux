...
__no_kmsan_checks
static void save_trace_log_lvl(struct task_struct *task, struct pt_regs *regs,
			unsigned long *stack, char *buf)
{
	struct unwind_state state;
	struct stack_info stack_info = {0};
	unsigned long visit_mask = 0;
...
void save_stack(char *buf, int max_len);
void save_stack(char *buf, int max_len)
{
	unsigned long *sp = get_stack_pointer(current, NULL);

	if (!buf)
		return;

	memset(buf, 0, max_len);
	save_trace_log_lvl(current, NULL, sp, buf);
}
